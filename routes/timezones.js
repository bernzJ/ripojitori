...
router.post("/timezones", requireJwtAuth, requireScope, async (req, res) => {
  try {
    ...
    const result = await new sql.Request().query(`SELECT * FROM [dbo].[Timezones] FOR JSON AUTO;`);
    res.send({
...
router.post("/timezones/create", requireJwtAuth, requireScope, ((req, res, next) => req.user.scope <= scopes.PLEB ? res.send({ message: "permission denied." }) : next()), async (req, res) => {
...
router.post("/timezones/del", requireJwtAuth, requireScope, ((req, res, next) => req.user.scope !== scopes.ADMIN ? res.send({ message: "permission denied." }) : next()), async (req, res) => {
  const ps = new sql.PreparedStatement();
  try {
