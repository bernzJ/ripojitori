...
router.post("/oms", requireJwtAuth, requireScope, async (req, res) => {
  try {
    ...
    const result = await new sql.Request().query(`SELECT Id, Type FROM [dbo].[OMS] FOR JSON AUTO;`);
    res.send({
...
router.post("/oms/create", requireJwtAuth, requireScope, ((req, res, next) => req.user.scope <= scopes.PLEB ? res.send({ message: "permission denied." }) : next()), async (req, res) => {
...
router.post("/oms/del", requireJwtAuth, requireScope, ((req, res, next) => req.user.scope !== scopes.ADMIN ? res.send({ message: "permission denied." }) : next()), async (req, res) => {
  const ps = new sql.PreparedStatement();
  try {
